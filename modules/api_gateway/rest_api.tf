...
resource "aws_api_gateway_rest_api" "rest_api" {
  ...
  description = "teraform serverless application"
  endpoint_configuration {
    types = ["REGIONAL"]
  }
}

# create resources in root
...
# create integration method to resource
resource "aws_api_gateway_integration" "rest_api_get_method_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = var.lambda_login_function_invoke_arn
}


# create response on resourse -> method with status code 200
...
# create response on resource -> method -> status code
...
##### lambda permishions
#########################
# create a lambda resource based policy to allow api gateway to invoke the lambda function
resource "aws_lambda_permission" "lambda_login_function_permission" {
...
# create deployment and triggers
